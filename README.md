refx-pp-py is a fork from MaxOhn's [rosu-pp-py]

Library to calculate difficulty and performance attributes for all [osu!] modes.

This is a python binding to the [Rust] library [refx-pp-rs] which was bootstrapped through [PyO3].
As such, its performance is much faster than a native python library.

[rosu-pp-py]: https://github.com/MaxOhn/rosu-pp-py
[PyO3]: https://github.com/PyO3/pyo3
[Rust]: https://www.rust-lang.org
[refx-pp-rs]: https://github.com/refx-online/refx-pp-rs